...
export default function Intake() {
  ...
  function handleGetLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocationStatus(
          'Unable to fetch location. You can still continue with default location.'
        );
      },
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-4">
      <div className="w-full max-w-[700px] bg-white rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-12">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-[28px] font-bold text-[#1A237E]">New Patient Assessment</h1>
        </div>
        <p className="text-sm text-slate-600 mb-6">
          Enter patient vitals — AI risk analysis in under 10 seconds
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className={`border-l-4 rounded-md p-2 ${statusClasses(getStatus('systolicBP', systolicBP))}`}>
              ...
              <span
                className={`mt-2 inline-block text-xs font-semibold px-2 py-1 rounded-full ${pillClasses(
          ...
          <button
            type="submit"
            className="w-full h-14 bg-[#C62828] hover:bg-red-700 text-white font-semibold rounded-xl text-lg flex items-center justify-center gap-2"
            disabled={submitting}
          >
            {submitting ? (
              <>
